...
if (isset($_GET['s'])) {
	$MEMCACHE_SERVERS = explode(';', $_GET['s']);
} else {
	$MEMCACHE_SERVERS[] = '10.250.14.11:11211'; // add more as an array
	$MEMCACHE_SERVERS[] = '10.250.14.12:11211'; // add more as an array
}
...
function sendMemcacheCommand($server,$port,$command){
	...
	if (!$s){
		if ($iKey = array_search($server, $MEMCACHE_SERVERS)) {
			unset($MEMCACHE_SERVERS[$iKey]);
		}
		$res = array("STAT" => array('pid' => 0,
		'uptime' => 0,
		'time'=> 0,
		'version'=> 0,
		'pointer_size' => 0,
...
function getMemcacheStats($total=true){
	...
	if ($total){
		...
		foreach($resp as $server=>$r){
			if (is_array($r) && count($r) && isset($r['STAT'])){
